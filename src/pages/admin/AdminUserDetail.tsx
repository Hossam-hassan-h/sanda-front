...
import { useUserQuery, useVerifyUser, useUnverifyUser } from "@/hooks/useAdminQueries";
...
export default function AdminUserDetail() {
  ...
  /** Persist the admin's review decision to the global map and the user object */
  const persistReview = (decision: "approved" | "rejected", reason?: string) => {
    ...
    // Update the user's localStorage if they're the currently-logged-in user
    try {
      const stored = localStorage.getItem("sanda_user");
      if (stored) {
        const u = JSON.parse(stored);
        if (u.id === user.id) {
          u.verificationRequest = reviewed;
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      // Use the API mutation to flip isVerified to true
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      // Make sure isVerified is false (could be true from previous review)
      if (user.isVerified) {
  ...
  // Use the user.verificationRequest from the API (or local storage override)
  // The rich mock users from the admin API may not have verificationRequest set,
  // so we also check the global localStorage map.
                    ...
                    <div className="border-t border-border pt-4 space-y-3">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-3 space-y-2">
                          ...
                          <Textarea placeholder="اكتب سبب الرفض (مثال: الصورة غير واضحة)" value={rejectionReason} onChange={(e) => setRejectionReason(e.target.value)} rows={2} className="text-xs" />
                    ...
                    <div className="border-t border-border pt-4">
                      <Button
                        ...
                        onClick={() => {
                          ...
                          map[user.id] = reset;
                          localStorage.setItem(VERIFICATION_REQUESTS_KEY, JSON.stringify(map));
                          toast({ title: "تمت إعادة فتح الطلب", description: "يمكنك مراجعة المستندات مرة أخرى" });
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-3 text-xs text-muted-foreground mt-1">
                            ...
                            <span className="flex items-center gap-1">{job.employerId === user.id ? "نشر" : "منفذ"}</span>
                          </div>
                        </div>
                        <Badge variant={job.status === "open" ? "default" : job.status === "in-progress" ? "secondary" : "outline"} className="shrink-0">
