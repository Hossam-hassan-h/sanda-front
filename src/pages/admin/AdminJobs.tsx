...
export default function AdminJobs() {
  ...
  useEffect(() => {
    if (!query.isLoading && page > totalPages) setPage(totalPages);
  ...
  const columns = useMemo<Column<Job>[]>(() => [
    ...
    },
    {
      ...
      header: "الميزانية",
      render: (j) => (
        <span className="font-semibold text-primary">{formatNumber(j.price)} ج</span>
      ),
    },
    {
      key: "applicantsCount",
  ...
  return (
    <AdminLayout>
                ...
                <Card className="border border-border">
                  <CardContent className="p-4">
                    ...
                    <div className="flex items-center gap-2 text-xs text-muted-foreground mb-3">
                      ...
                      <span className="font-semibold text-primary">{formatNumber(j.price)} ج</span>
                      <span className="inline-flex items-center gap-1"><User className="h-3 w-3" />{j.applicantsCount ?? 0}</span>
                    </div>
      ...
      <JobEditModal
        open={!!editJobId}
        onOpenChange={(open) => { if (!open) setEditJobId(null); }}
        job={editingJob}
