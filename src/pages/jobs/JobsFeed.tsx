...
export default function JobsFeed() {
  ...
  const filters = useMemo(
    () => ({ q: debouncedQ || undefined, city, category, page, limit: PAGE_SIZE }),
    [debouncedQ, city, category, page],
  );
  const { data: response, isLoading, isError } = usePaginatedJobs(filters);

  const jobs = response?.data ?? [];
  const totalJobs = response?.total ?? 0;
  const totalPages = response?.totalPages ?? 1;
  const currentPage = response?.page ?? page;
  const pageStart = totalJobs === 0 ? 0 : (currentPage - 1) * PAGE_SIZE + 1;
  const pageEnd = Math.min(currentPage * PAGE_SIZE, totalJobs);
      ...
      <section className="container mx-auto px-4 md:px-6 py-10">
        <div className="flex items-center justify-between mb-6 gap-4 flex-wrap">
          ...
          {!isLoading && totalJobs > 0 && (
            <p className="text-sm text-muted-foreground">
              عرض {pageStart}–{pageEnd} من {totalJobs}
            </p>
          )}
        </div>
        ...
        {isError ? (
          ...
          <div className="text-center py-20">
            ...
            <button
              onClick={() => { setQ(""); setCity("all"); setCategory("all"); setPage(1); }}
