...
export const jobsApi = {
  async listPaginated(filters: JobFilters = {}): Promise<JobsListResponse> {
    if (USE_MOCKS) {
      let result = [...mockJobs];
      if (filters.q) {
    ...
    if (filters.q) backendParams.q = filters.q;
    ...
    const pagination = (rawBody.pagination ?? {}) as Record<string, unknown>;
    const jobs = ((rawBody.data ?? []) as Record<string, unknown>[]).map(mapJob);
    const limit = Number(pagination.limit ?? filters.limit ?? 10) || 10;
    const total = Number(pagination.total ?? pagination.totalItems ?? jobs.length) || 0;

    return {
      data: jobs,
      total,
      page: Number(pagination.page ?? filters.page ?? 1) || 1,
      limit,
      totalPages: Number(pagination.totalPages ?? Math.max(1, Math.ceil(total / limit))) || 1,
    };
  },
  ...
  async list(filters: JobFilters = {}): Promise<Job[]> {
    ...
    if (filters.city && filters.city !== "all") backendParams.location = filters.city;
    if (filters.page) backendParams.page = filters.page;
    backendParams.limit = filters.limit ?? 100;
    const { data: body } = await api.get("/jobs", { params: backendParams });
